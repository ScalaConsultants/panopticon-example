...
object GraphQLApi extends GenericSchema[ItemRepository] {
  ...
  private final case class Queries(
    ...
    itemByName: ItemName => RIO[ItemRepository, List[Item]],
    @GQLDescription("Find items cheaper than specified price")
    cheaperThan: ItemPrice => RIO[ItemRepository, List[Item]],
    @GQLDescription("Get all items")
    allItems: RIO[ItemRepository, List[Item]],
    @GQLDescription("Get item by ID")
    item: ItemId => RIO[ItemRepository, Option[Item]]
  )

  private final case class Mutations(
    addItem: ItemInput => RIO[ItemRepository, ItemId],
    deleteItem: ItemId => RIO[ItemRepository, Unit]
  )
  ...
  private def runService[A](f: ZIO[ItemRepository, DomainError, A]): RIO[ItemRepository, A] =
    f.mapError(_.asThrowable)

  private val api: GraphQL[Clock with Logging with ItemRepository] = {
    graphQL(
      RootResolver(
        Queries(
  ...
  val live =
    ZLayer.fromServicesM[
      ...
      GraphQLApi.Service
    ] { (logger, itemRepositoryService, actorSystem) =>
      implicit val system  = actorSystem
      implicit val ec      = system.dispatcher
      implicit val runtime = Runtime.default.map(_ ++ Has(logger) ++ Has(itemRepositoryService))
